...
float         dist[<i,j> in Edges] = sqrt((posx[i]-posx[j])^2+(posy[i]-posy[j])^2);
...
execute {
  ...
  if (newSubtourSize != n)
    ...
    for(var k=1;k<=n;k++)
    {
              writeln(kcity);
              solution.add(kcity);
              xstring=xstring+posx[kcity];
              if (k!=n) xstring=xstring+",";
              ystring=ystring+posy[kcity];
              if (k!=n) ystring=ystring+",";
              cstring=cstring+"2";
              if (k!=n) cstring=cstring+",";
              kcity=thisSubtour[kcity]; 
    ...
    1;
    writeln(solution);
    var output=new IloOplOutputFile("tspresult.dat");
